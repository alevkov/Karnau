...
- (void) willMoveToSuperview:(UIView *)newSuperview{
    ...
    [self moveCircleToAngle:0 with:self.switchCompletion];
...
- (void) moveCircleToAngle: (double)angle with:(Handler)completion{
    self.angle = angle;
	int previousNum = self.currentNum;
    [self setNeedsDisplay];
	...
	if (!_letterScale) {
		...
		self.numberLabel.text = [self convertNumberToLetterScale:self.currentNum];
		//completion();
	}
	
	if (self.currentNum != previousNum) {
		if (completion != nil) {
			completion(self.currentNum);
		}
...
-(NSString*)convertNumberToLetterScale:(int)num {
	char l[15] = "ABCDEFGHIJK";
	NSMutableString *output = [[NSMutableString alloc]initWithCapacity:5];
	
	for (int i = 0; i <= num; i++) {
		[output appendString:[NSString stringWithFormat:@"%c", l[i]]];
...
- (void) handlePan:(UIPanGestureRecognizer *)pv {
    ...
    [self moveCircleToAngle:angle with:self.switchCompletion];
}

-(void) handleManualTranslation: (CGPoint) withPoint {
	CGPoint translation = withPoint;
	CGFloat x_displace = translation.x - self.trueCenter.x;
	CGFloat y_displace = -1.0*(translation.y - self.trueCenter.y);
	double radius = pow(x_displace, 2) + pow(y_displace, 2);
	radius = pow(radius, .5);
