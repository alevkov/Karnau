...
class QMMinterm : NSObject {
  ...
  init(i: UInt, m: UInt) {
    if i == implicantFlag {
      ...
      if m <= maxMagnitude {
        ...
        self.stringValue = "0" * (m - UInt(self.stringValue.count)) + self.stringValue
        for s in self.stringValue {
  ...
  convenience init (min1: QMMinterm, min2: QMMinterm, idx: UInt) {
    ...
    s.replaceSubrange(s.index(s.startIndex, offsetBy: Int(s.count - 1) - Int(idx))...s.index(s.startIndex, offsetBy: Int(s.count - 1) - Int(idx)), with: "-")
    self.stringValue = s
  }
  
  convenience init(str: String, magnitude: UInt) {
    self.init(i: primeFlag, m: magnitude)
    self.magnitude = UInt(str.count)
  }
